...
export default function Login() {
    ...
    const { isAuthenticated, role, login, setJwtUser } = useAuth();
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (res.token) {
                ...
                if (setJwtUser) setJwtUser(res.user, res.user?.role || userType);
                ...
                navigate('/verify-otp', { state: { email: cleanEmail, mode: 'login', userType, uid: id } });
            }

        } catch (error) {
            console.error(error);
            const msg = error.error || error.message || "Incorrect email or password. Please try again or register.";
            toast.error(msg);
        } finally {
            setIsLoading(false);
        }
